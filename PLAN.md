# Build Plan — Survival Trivia Game (working title: TBD)

**For:** AI coding agent
**Platform:** Portlyy (existing TanStack Start + Bun + Cloudflare Workers/R2 codebase)
**Conforms to:** the Portlyy game-module convention (`src/games/<game-id>/`), static-bank-on-R2 pattern, no DB, no auth, localStorage persistence. Mirrors how Nearo is built.

> **Game ID placeholder:** this doc uses `<game-id>` throughout. Name is decided separately; substitute the final id before scaffolding.

Game name would be Tickr
...
>
> Eventually the clock hits **zero** and the run ends. If jumpscare mode is **on**, a brief mild scare plays first. Then I see my **result**: how many I got **right** (the headline), plus **time survived** and **best streak**. If I beat my **personal best for that timer preset**, I get a "New best!" celebration. A big **Play Again** lets me go straight back.
...
**Scoring / results**
- **Correct count = headline score.** Time survived + best streak = secondary stats.
...
- **Assets are owner-supplied (added later).** Build against an **asset manifest + interface** with a safe placeholder. Asset requirements written into the contract: must be **licensed/owned** (no ripped copyrighted clips) and **flash-safe** (no rapid strobe). The agent does **not** source assets.

---

## 4. Architecture (conform to Portlyy convention)

Game module at `src/games/<game-id>/`:

```
src/games/<game-id>/
├── <game-id>.tsx           # Entry component — start screen ↔ run ↔ result orchestration
...
│   └── jumpscare-overlay.tsx   # plays the scare asset (or placeholder)
├── hooks/
...
- Scaffold `src/games/<game-id>/` per convention; add route `src/routes/games/<game-id>.tsx`; register in `src/features/home/lib/games.ts` (id, name, category: Trivia, glyph, status, description, meta); add `config.ts` with timer presets, time deltas, difficulty thresholds, storage prefix `portlly:<game-id>`.
...
- New script under `scripts/preprocess/` (sibling to the Nearo pipeline), wired to a `bun run preprocess:<game-id>` command.
...
- Output **difficulty-bucketed files**: `<game-id>/easy.json`, `medium.json`, `hard.json`. Schema per item: `{ id, question, correct, options[4], category, difficulty }`. (Pre-shuffle options or shuffle client-side — pick one and be consistent.)
...
### Phase 6 — Jumpscare mode (opt-in, safety-gated)
...
### Phase 7 — SEO, tests, polish, deploy
- `seo.ts`: VideoGame + HowTo + FAQ structured data; add route to sitemap.
- **E2E (Playwright)** `tests/e2e/<game-id>.spec.ts` using smoke fixtures: pick preset → answer correct (time up) → answer wrong (time down) → reach zero → result + best; jumpscare toggle shows consent modal; mode-off path skips the scare.
...
Scare assets dropped in later must be: **owned or royalty-free/CC0** (no ripped copyrighted YouTube/movie/game/cartoon clips), **mild**, and **flash-safe** (no rapid strobe), with a synchronized **sound sting**. Until provided, the placeholder ships and tests run against it.
